...
namespace PhoenixProject.Database
{
    public class MySqlCommand
    {
        ...
        public int Execute()
        {
            ...
            using (var conns = Database.DataHolder.MySqlConnection)
            {
                try
                {
                    if (conns.State == System.Data.ConnectionState.Open)
                        conns.Close();
                    if (conns.State == System.Data.ConnectionState.Closed)
                        conns.Open();
                    MySql.Data.MySqlClient.MySqlCommand cmd = new MySql.Data.MySqlClient.MySqlCommand(Command, conns);
                    Program.PoolSize++;
