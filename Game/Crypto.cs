...
namespace ConsoleApplication1
{
    ...
    {
        /* Swap the nybbles */
        public static byte Rot8(byte rValue)
        {
            return (byte)(rValue << 4 | rValue >> 4);
        }

        /* 32bit rotate left */
        public static UInt32 Rol32(UInt32 rValue, int rAmount)
        {
            return (UInt32)(rValue << rAmount | rValue >> 32 - (rAmount));
        }

        /* 32bit rotare right */
        public static UInt32 Ror32(UInt32 rorValue, int rorAmount)
        {
            return (UInt32)(rorValue >> rorAmount | rorValue << 32 - (rorAmount));
        }
    }
    class Program
    {
        private static byte[] co_cryptkey1 = new byte[256] 
        {
        ...
        private static byte[] co_cryptkey2 = new byte[256] 
        {
        ...
        static byte[] PerformPasswordCrypto(string username, string password)
        {
            return ASCIIEncoding.ASCII.GetBytes("Work the crypto out yourself. if you have decrypted this, then you are one step closer :)");
        }
        static void Main(string[] args)
        {
            PhoenixProject.Console.WriteLine(ASCIIEncoding.ASCII.GetString(PerformPasswordCrypto("username", "password")));
        }
    }
}